...
pub fn print_runtime_info(runtime_config: &WatchdogRuntimeConfig) {
    ...
    println!("🎥 Target Movies folder: {:?}", runtime_config.target_base_movie_folder);
    println!("📺 Target Series folder: {:?}", runtime_config.target_base_series_folder);
...
pub fn get_runtime_info() -> Result<WatchdogRuntimeConfig> {
    ...
    let watch_folder_str = env::var("SMO_WATCHDOG_WATCH_FOLDER")
        .context("SMO_WATCHDOG_WATCH_FOLDER is not set")?;
