...
mod tests {
    ...
    fn sample_function_source() -> &'static str {
        "fn sample() -> &'static str { \"ok\" }"
    }

    #[test]
    fn system_message_is_non_empty_and_describes_role() {
        let message = build_rust_ai_function_system_message();

        assert!(!message.is_empty());
        assert!(message.contains("You are an AI"));
    }

    #[test]
    fn user_message_embeds_function_source_and_input() {
        let message = build_rust_ai_function_user_message(sample_function_source, "the-input");

        assert!(message.contains("fn sample()"));
        assert!(message.contains("the-input"));
