...
## 3.0.0
- BREAKING: `MockOptions` gained a `locale: Locale` field, so literal construction needs the
  new field (use `Locale::default()` for the old behavior; every existing generator output is
  unchanged under the default `en`).
- New `Locale` enum covering the 14 locales fake 5.1.0 ships (`en`, `ar-sa`, `cy-gb`, `de-de`,
  `fa-ir`, `fr-fr`, `it-it`, `ja-jp`, `nl-nl`, `pt-br`, `pt-pt`, `tr-tr`, `zh-cn`, `zh-tw`),
  with `from_code` (case-insensitive, `-`/`_` interchangeable), `code`, `all_codes`, and
  `Display`.
- The fake-backed generators honor the locale: the `person.*` types except birthday,
  `internet.username`, `commerce.company`, `commerce.job-title`, and `commerce.industry`.
  `person.email` localizes the name parts; its domain list stays fixed, so non-Latin locales
  produce non-ASCII local parts. fake falls back to its English data for categories a locale
  does not localize, so every locale is valid for every type.
- New `DataType::is_locale_aware` reports whether a type honors the locale; tool-mock uses it
  to warn instead of silently ignoring `--locale` on English-only types.
- Tests: locale code round-trips, unknown-code error, the locale-aware set, per-locale
  generation, and a zh-cn non-ASCII tripwire pinning that dispatch switches data sets.

## 2.0.0
- The `generators` module is no longer public. Its functions assume options
  pre-validated by `generate_mock_data` and could panic on out-of-range input
  when called directly; sealing the module makes `generate_mock_data` (plus
  `models`) the whole public API, so unvalidated options can no longer reach a
  generator. The only known consumer, tool-mock, already used only that entry
  point and needs no change.
