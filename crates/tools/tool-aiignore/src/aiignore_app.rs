...
mod tests {
    ...
    #[test]
    fn sanitize_aiignore_data_drops_comments_and_blanks_then_sorts() {
        let mut input = HashSet::new();
        input.insert("# comment".to_string());
        input.insert("".to_string());
        input.insert("   ".to_string());
        input.insert("secret.key".to_string());
        input.insert("  *.env  ".to_string());
        input.insert("secret.key".to_string());

        let result = sanitize_aiignore_data(&input);

        assert_eq!(result, vec!["*.env".to_string(), "secret.key".to_string()]);
    }
}
