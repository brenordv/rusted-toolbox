...
pub fn load_chat_personality_prompt(personalities_path: &str) -> Result<(String, String)> {
    ...
    let personality_name = extract_personality_name(&selected_personality_file);
...
/// Extracts the personality name from a personality file name.
///
/// The name is the segment after the last underscore, with any file extension removed
/// (e.g. `chat_prompt_Alice.txt` yields `Alice`).
fn extract_personality_name(file_name: &str) -> String {
    file_name
        .split('_')
        .next_back()
        .unwrap_or("")
...
mod tests {
    ...
    #[test]
    fn extract_personality_name_strips_prefix_and_extension() {
        assert_eq!(extract_personality_name("chat_prompt_Alice.txt"), "Alice");
    }

    #[test]
    fn extract_personality_name_without_underscore_uses_whole_stem() {
        assert_eq!(extract_personality_name("Bob.md"), "Bob");
    }

    #[test]
    fn extract_personality_name_uses_last_segment() {
        assert_eq!(extract_personality_name("chat_bot_persona_Zoe.json"), "Zoe");
    }

    #[test]
    fn extract_personality_name_without_extension() {
        assert_eq!(extract_personality_name("persona_Max"), "Max");
    }

    #[test]
    fn extract_personality_name_empty_input_yields_empty() {
        assert_eq!(extract_personality_name(""), "");
    }
}