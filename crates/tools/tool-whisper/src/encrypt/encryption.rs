...
mod tests {
    ...
    #[test]
    fn new_keypair_roundtrips_and_fingerprint_survives_reserialization() {
        let keypair = Encryption::new_keypair().unwrap();

        let pub_b64 = keypair.get_public_key().unwrap();
        let recreated = Encryption::create_pub_key_from_base64(&pub_b64).unwrap();
        let encrypter = MessageEncrypter::new(recreated).unwrap();

        assert_eq!(
            keypair.get_pub_key_fingerprint().unwrap(),
    ...
    #[test]
    fn create_pub_key_from_base64_rejects_invalid_input() {
        assert!(Encryption::create_pub_key_from_base64("!!! not base64 !!!").is_err());
    }
}
