...
mod tests {
    ...
    #[test]
    fn fix_cli_command_as_string_exposes_function_source() {
        let source = fix_cli_command_as_string();

        assert!(!source.trim().is_empty());
        assert!(source.contains("fix_cli_command"));
    }

    #[test]
    fn suggest_cli_command_as_string_exposes_function_source() {
        let source = suggest_cli_command_as_string();

        assert!(!source.trim().is_empty());
        assert!(source.contains("suggest_cli_command"));
    }
}
