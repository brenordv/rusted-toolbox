...
mod tests {
    ...
    #[test]
    fn build_os_info_includes_shell_when_present() {
        let info = build_os_info("linux", &Some("bash".to_string()));

        assert_eq!(info, "OS: linux (shell: bash)");
    }

    #[test]
    fn build_os_info_omits_shell_when_absent() {
        let info = build_os_info("windows", &None);
